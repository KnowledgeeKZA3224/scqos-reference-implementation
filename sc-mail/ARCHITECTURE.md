...
SC Mail never schedules more than 9,950 SC-managed recipients inside the configured rolling 24-hour Outlook window. This is deliberately below the 10,000-recipient Exchange Online hard limit. Internal pacing is configured below the 30-message/minute hard limit.
...
## Non-goals
SC Mail does not attempt to evade spam systems, falsify consent/origin, or hide abusive traffic. Account protection comes from proving legitimate origin, authorization, authentication, suppression, pacing and current provider boundaries before each consequence.
